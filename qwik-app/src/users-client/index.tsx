...
export const getUsers = server$(async (): Promise<User[] | Error> => {
...
export const createUser = server$(
  async (username: string): Promise<User | Error> => {
    let response: Response;
    try {
      response = await fetch(`http://127.0.0.1:3000/users`, {
        method: "POST",
...
export const deleteUserById = server$(
...
export const updateUserById = server$(
  async (id: string, username: string): Promise<User | Error> => {
    let response: Response;
    try {
      response = await fetch(`http://127.0.0.1:3000/users/` + id, {
        method: "PUT",
